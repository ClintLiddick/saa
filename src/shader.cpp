...
Shader::Shader(const std::string &vert_shader_path, const std::string &frag_shader_path) {
    ...
    if (vert_str.empty()) {
        ...
        ss << "Failed to load vertex shader: " << vert_shader_path;
    ...
    if (frag_str.empty()) {
        std::ostringstream ss;
        ss << "Failed to load fragment shader: " << frag_shader_path;
        throw std::runtime_error{ss.str()};
    }
...
void Shader::set_uniformf(const std::string &name, const float value) {
    ...
    if (location < 0) {
        throw std::runtime_error{"Failed to find \"" + name + "\" uniform in shader."};
    }
    glUniform1f(location, value);
}

void Shader::set_clip_from_local(const Mat4f &clip_from_local) {
    const GLint clip_from_local_location = glGetUniformLocation(program_, "clip_from_local");
    if (clip_from_local_location < 0) {
        throw std::runtime_error{"Failed to find \"clip_from_local\" uniform in shader."};
    }
    glUniformMatrix4fv(clip_from_local_location, 1, GL_FALSE, clip_from_local.data());
}
